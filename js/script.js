...
function hitungTotalbali() {
  ...
  document.getElementById("totalHarga-bali").innerText =
...
function hitungTotalbromo() {
  ...
  // Menampilkan total harga pada span dengan id 'totalHarga'
  //totalHarga.toLocaleString('id-ID'); digunakan untuk mengubah nilai totalHarga menjadi format string dengan pemisah ribuan dan menggunakan tanda desimal sesuai dengan konvensi bahasa Indonesia ('id-ID').
  document.getElementById("totalHarga-bromo").innerText =
    totalHarga.toLocaleString("id-ID");
}
...
function hitungTotallabuanbajo() {
  ...
  // Menampilkan total harga pada span dengan id 'totalHarga'
  //totalHarga.toLocaleString('id-ID'); digunakan untuk mengubah nilai totalHarga menjadi format string dengan pemisah ribuan dan menggunakan tanda desimal sesuai dengan konvensi bahasa Indonesia ('id-ID').
  document.getElementById("totalHarga-labuanbajo").innerText =
    totalHarga.toLocaleString("id-ID");
}
...
async function fetchData() {
  try {
    const response = await fetch(
      "https://be-2-jakarta-5-production.up.railway.app/destinationPackage/get"
    );
...
async function updateDestinasiPackages() {
  ...
  destinasiData.forEach((destinasi, index) => {
    ...
    card.innerHTML = `
      ...
      <a href="${
        Link[index].link_url
      }" target="_blank" class="button-destinasi">Book Now</a>
    `;
...
function generateStarRating(rating) {
  ...
  for (let i = 1; i <= maxRating; i++) {
    const starClass =
      i <= rating ? "fa-solid fa-star fa-lg" : "fa-solid fa-star fa-lg";
...
async function newsdata() {
  try {
    const response = await fetch(
      "https://be-2-jakarta-5-production.up.railway.app/news/get"
    );
    const data = await response.json();
    return data;
  } catch (error) {
